...
func setValidator(t *testing.T, f *deterministicFixture, validator stakingtypes.Validator) {
	...
	_, err = f.stakingKeeper.DelegateInternal(f.ctx, delegatorAddress, validator.BondedTokens(), stakingtypes.Unbonded, validator, true)
	assert.NilError(t, err)
}
...
func fundAccountAndDelegate(
	...
	shares, err := f.stakingKeeper.DelegateInternal(f.ctx, delegator, amt, stakingtypes.Unbonded, validator, true)
	return shares, err
}
...
func TestGRPCValidatorUnbondingDelegations(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		for i := 0; i < numDels; i++ {
			...
			valbz, err := f.stakingKeeper.ValidatorAddressCodec().StringToBytes(validator.GetOperator())
			assert.NilError(t, err)
			_, _, err = f.stakingKeeper.UndelegateInternal(f.ctx, delegator, valbz, shares)
			assert.NilError(t, err)
		}
	...
	_, _, err = f.stakingKeeper.UndelegateInternal(f.ctx, delegatorAddr1, validatorAddr1, shares1)
	assert.NilError(t, err)
	...
	_, _, err = f.stakingKeeper.UndelegateInternal(f.ctx, delegatorAddr2, validatorAddr1, shares2)
	assert.NilError(t, err)
