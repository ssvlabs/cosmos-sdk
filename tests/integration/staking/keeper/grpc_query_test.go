...
func TestGRPCQueryUnbondingDelegation(t *testing.T) {
	...
	_, _, err := f.stakingKeeper.UndelegateInternal(ctx, addrAcc2, valAddr, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
...
func TestGRPCQueryDelegatorUnbondingDelegations(t *testing.T) {
	...
	assert.NilError(t, err1)
	_, _, err := f.stakingKeeper.UndelegateInternal(ctx, addrAcc, valAddr1, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
	valAddr2, err1 := sdk.ValAddressFromBech32(addrVal2)
	assert.NilError(t, err1)
	_, _, err = f.stakingKeeper.UndelegateInternal(ctx, addrAcc, valAddr2, math.LegacyNewDecFromInt(unbondingTokens))
	assert.NilError(t, err)
...
func TestGRPCQueryValidatorUnbondingDelegations(t *testing.T) {
	...
	assert.NilError(t, err)
	_, _, err = f.stakingKeeper.UndelegateInternal(ctx, addrAcc1, valbz, math.LegacyNewDecFromInt(undelAmount))
	assert.NilError(t, err)
	applyValidatorSetUpdates(t, ctx, f.stakingKeeper, -1)
